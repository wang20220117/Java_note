...
* `html` 元素囊括了页面中所有其它元素，*整个页面只需一个*，称为**根元素**
...
* method 请求方式有 
  * get （默认）提交时，**数据跟在 URL 地址之后**  (get没有请求体)
  * post 提交时，**数据在请求体内**
...
1. 请求行（请求方式，URI，协议版本） 
   ...
   1. > URI（Uniform Resource Identifier，统一资源标识符）是用于标识和定位互联网资源的字符串序列。它是一个通用的术语，包括两个具体的实现：URL（Uniform Resource Locator，统一资源定位符）和URN（Uniform Resource Name，统一资源名称）。
...
可以上传文件，而url编码方式就不能上传文件。

javascript里反引号才支持换行，单引号和双引号都不支持。
...
* 表单只支持以 application/x-www-form-urlencoded 和 multipart/form-data 格式发送数据，json格式需要使用 javascript
* 文件上传需要用 multipart/form-data 格式
* js 代码可以支持任意格式发送数据 

服务端接收

* 对 application/x-www-form-urlencoded 和 multipart/form-data 格式的数据，Spring 接收方式是统一的，只需要**用 java bean** 的属性名对应请求参数名即可
...
### 4) jwt 原理 （JSON Web Token）

jwt 技术实现身份验证(适合分布式项目)
